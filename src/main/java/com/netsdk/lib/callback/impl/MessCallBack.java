...
public class MessCallBack implements NetSDKLib.fMessCallBack {
    ...
    public boolean invoke(int lCommand, LLong lLoginID, Pointer pStuEvent,
        ...
        switch (lCommand) {
            ...
            case NetSDKLib.NET_ALARM_KEYPAD_TAMPER: // 键盘防拆报警/恢复事件
            {
                ALARM_KEYPAD_TAMPER_INFO msg = new ALARM_KEYPAD_TAMPER_INFO();
                ToolKits.GetPointerData(pStuEvent, msg);
                System.out.printf("【键盘防拆报警/恢复事件】 时间:%s 电话线序号:%d 事件动作(0:开始 1:停止):%d \n",
                        msg.stuTime, msg.nChannelID, msg.nAction);
                break;
            }
            ...
            case SDK_ALARM_Ex_TYPE.SDK_ALARM_NO_DISK: {
                ...
                ToolKits.GetPointerData(pStuEvent, info);
                System.out.println("无硬盘报警-时间:" + info.stuTime.toString() + ",动作:" + (info.dwAction == 0 ? "开始" : (info.dwAction == 1 ? "停止" : "未知")));
                break;
            }
            /************************************************************
             *                      热成像测温点报警                       *
