...
public class ArmDisarmParamConfigDemo {
    ...
    // 回调函数需要是静态的，防止被系统回收
    // 断线回调
    private static NetSDKLib.fDisConnect disConnectCB = DefaultDisConnect.GetInstance();
    // 重连回调
    private static NetSDKLib.fHaveReConnect haveReConnectCB = DefaultHaveReconnect.getINSTANCE();
    ...
    public void loginWithHighLevel() {
        ...
        if (m_hLoginHandle.longValue() == 0) {
            ...
            log.info("Device Address：" + m_strIpAddr);
            log.info("DVR报警输入个数：" + deviceInfo.byAlarmInPortNum);
        }
    }
    ...
    public void queryAlarmChnCount() {
        ...
        count.read();
        if (!bRet) {
            log.error("查询报警通道数失败：" + ToolKits.getErrorCodeShow());
            return;
        }
        log.info("本地报警输入通道数:" + count.nLocalAlarmIn);
        log.info("远程报警输入通道数:" + count.nRemoteAlarmIn);
    }
    ...
    public void queryAlarmInChn() throws UnsupportedEncodingException {
        ...
        if (!bRet) {
            log.error("查询报警输入通道信息失败：" + ToolKits.getErrorCodeShow());
            return;
        }
        ToolKits.GetPointerDataToStructArr(params, infos);
        int nRetNum = intRetLen.getValue() / infos[0].size();
        log.info("返回个数: " + nRetNum);
        for (int i = 0; i < nRetNum; ++i) {
            log.info("通道: " + i);
            NET_ALARM_IN_CHANNEL info = infos[i];
            log.info("设备ID:" + new String(info.szDeviceID));
            log.info("报警通道名称:" + new String(info.szName, encode));
        }
    }
    ...
    public void setBypass() {
        ...
        if (flg) {
            ...
            log.error("设置旁路功能失败:" + ToolKits.getErrorCodeShow());
        }
